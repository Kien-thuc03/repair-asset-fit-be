...
async function seedSoftwareBuildingH() {
  ...
  try {
    ...
    for (let i = 0; i < computers.length; i++) {
      ...
      const optionalSoftware = SOFTWARE_LIST.filter(
        (sw) => !MANDATORY_SOFTWARE.includes(sw.name)
      )
        ...
        if (existing.length === 0) {
          await queryRunner.manager.query(
            ...
            [
              ...
              softwareId,
              installationDate,
              `Cài đặt ban đầu cho ${computer.roomCode} - Máy ${computer.machineLabel}`,
            ]
          );
          totalInstallations++;
        }
      ...
      if ((i + 1) % 50 === 0 || i === computers.length - 1) {
        console.log(
          `  📊 Progress: ${i + 1}/${computers.length} computers (${totalInstallations} installations)`
        );
      }
    }
    ...
    console.log(`📊 Summary:`);
    console.log(
      `   - Total software types: ${Object.keys(createdSoftwareIds).length}`
    );
    console.log(`   - Total computers: ${computers.length}`);
    console.log(`   - Total installations: ${totalInstallations}`);
    console.log(
      `   - Average per computer: ${(totalInstallations / computers.length).toFixed(1)} software`
    );
    ...
    console.log(
      `   - Total installations in Building H: ${totalInstallationsCheck[0].total}`
    );
    ...
    sampleComputers.forEach((row: any) => {
      console.log(
        `     ${row.roomCode} - Máy ${row.machineLabel}: ${row.software_count} software`
      );
    });
