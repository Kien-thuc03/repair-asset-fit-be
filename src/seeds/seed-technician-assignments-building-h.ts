...
async function seedTechnicianAssignments() {
  console.log('🌱 Starting seed script for Technician Assignments - Building H...\n');
  ...
  try {
    ...
    for (const assignment of TECHNICIAN_ASSIGNMENTS) {
      ...
      if (techExists.length === 0) {
        console.error(`❌ ERROR: Technician with ID ${assignment.technicianId} not found!`);
        console.log('   Please ensure the user exists before running this seed.\n');
        return;
      }

      console.log(`✅ Found: ${techExists[0].fullName} (${techExists[0].username})`);
    }
    console.log();
    ...
    for (const assignment of TECHNICIAN_ASSIGNMENTS) {
      console.log(`👤 Processing assignments for ${assignment.technicianName}:`);
    ...
    if (verifyAssignments.length > 0) {
      ...
      // Group by floor
      const floorGroups = verifyAssignments.reduce((acc, item) => {
        if (!acc[item.floor]) {
          acc[item.floor] = [];
        }
    ...
    console.log(`   - Total in Building H: ${verifyAssignments.length} assignments`);
    console.log('═══════════════════════════════════════════════════════\n');
