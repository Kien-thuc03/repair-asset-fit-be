...
export class RepairsController {
  ...
  @ApiOperation({
    ...
    description: `
      ...
      **Quyền hạn:**
      - Kỹ thuật viên: Có thể tiếp nhận yêu cầu trong phạm vi trách nhiệm
      - Tổ trưởng Kỹ thuật: Có thể tiếp nhận tất cả yêu cầu
      - Trưởng phòng quản trị: Có thể tiếp nhận bất kỳ yêu cầu nào
      
      **Điều kiện:**
      - Yêu cầu phải ở trạng thái CHỜ_TIẾP_NHẬN
      - Tài sản phải tồn tại và không bị xóa
      
      **Sau khi tiếp nhận:**
      - Tự động cập nhật acceptedAt timestamp
      - Có thể phân công kỹ thuật viên cụ thể
      - Có thể bắt đầu quá trình chẩn đoán và xử lý
      - Ghi log hoạt động vào RepairLogs
    `,
  })
  @ApiParam({
    name: "id",
  ...
  @ApiResponse({
    status: HttpStatus.OK,
    description: "Tiếp nhận yêu cầu sửa chữa thành công",
    type: RepairRequestResponseDto,
    schema: {
      example: {
        ...
        status: "ĐÃ_TIẾP_NHẬN",
        acceptedAt: "2025-10-23T10:30:15.234Z",
        computerAsset: {
  ...
  @ApiResponse({
    status: HttpStatus.BAD_REQUEST,
    description: "Yêu cầu không thể tiếp nhận",
    schema: {
      examples: {
        "wrong-status": {
          ...
          value: {
            statusCode: 400,
            message: "Chỉ có thể tiếp nhận yêu cầu ở trạng thái CHỜ_TIẾP_NHẬN",
            error: "Bad Request",
          },
        },
        "already-accepted": {
          summary: "Đã được tiếp nhận",
          value: {
            statusCode: 400,
            message: "Yêu cầu này đã được tiếp nhận bởi kỹ thuật viên khác",
            error: "Bad Request",
          },
        },
      },
  ...
  @ApiResponse({
    status: HttpStatus.FORBIDDEN,
    description: "Không có quyền tiếp nhận yêu cầu",
    schema: {
      example: {
        statusCode: 403,
        message: "Bạn không có quyền tiếp nhận yêu cầu sửa chữa",
        error: "Forbidden",
      },
    },
  })
  ...
  @ApiOperation({
    summary: "Lấy danh sách tầng được phân công",
    description: `
      Lấy danh sách tầng mà kỹ thuật viên hiện tại được phân công phụ trách.
      
      **Quyền hạn:**
      - Kỹ thuật viên: Xem tầng được phân công
      - Tổ trưởng Kỹ thuật: Xem tất cả tầng
      - Trưởng phòng quản trị: Xem tất cả tầng
      
      **Thông tin trả về:**
      - Danh sách building + floor
  ...
  @Get("by-floor")
  @ApiOperation({
    summary: "Lấy yêu cầu sửa chữa theo tầng",
    description: `
      Lấy danh sách yêu cầu sửa chữa trong tầng được phân công cho kỹ thuật viên.
      
      **Quyền hạn:**
      - Kỹ thuật viên: Chỉ xem yêu cầu trong tầng được phân công
      - Tổ trưởng Kỹ thuật: Xem tất cả yêu cầu
      - Trưởng phòng quản trị: Xem tất cả yêu cầu
      
      **Filter tự động:**
      - Kỹ thuật viên chỉ thấy yêu cầu trong building+floor được assign
  ...
  @ApiQuery({
    name: "building",
    required: false,
    description: "Lọc theo tòa nhà (A, B)",
    example: "A",
  })
  @ApiQuery({
    name: "floor",
    required: false,
    description: "Lọc theo tầng (1, 2, 3, 4, 5)",
    example: "1",
  })
  @ApiQuery({
    name: "status",
  ...
  @ApiResponse({
    status: HttpStatus.OK,
    description: "Lấy danh sách yêu cầu theo tầng thành công",
  })
  async getRequestsByFloor(
    ...
    return this.repairsService.findByFloor(filter, user);
  }
